...
class Breadcrumbs {
	...
	public function __construct( Config_Interface $config ) {
		...
		$this->bloginfo_name = $this->config['plugin']['bloginfo_name'];
		$this->home_url = $this->config['plugin']['home_url'];
	}
	...
	public function get_the_breadcrumbs( $args = array() ) {
		...
		$default = array(
			...
			'closed_wrapper'		=> '</ol>',
			'before_element'		=> '<li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">',
			'before_element_active'	=> '<li class="breadcrumb-item active" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">',
			'after_element'			=> '</li>',
